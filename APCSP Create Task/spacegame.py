...
def gameLoop():
    ...
    while gameRunning == True:
        ...
        gameOverDisplayText = gameOver.textDisplay()
        gameOverRect = gameOverDisplayText.get_rect()
        gameOverRect.center = ((gameWindowWidth//2), (gameWindowHeight//2))
        def gameOverMethod():
            score = 0
            gameWindow.blit(gameOverDisplayText, gameOverRect)
            pygame.display.update()
            time.sleep(2)
            gameLoop()
        ...
        playerScore = scoreText.textDisplay()
        def highScoreMethod():
